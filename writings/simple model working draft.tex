...
\begin{maxi*}[1]<b>
	...
	\addConstraint{ A'(t)}{ = r_t A(t)+ \bar{w}_t(L_I(t) + L_F(t)) - C(t)} {\text{Asset law of motion}}
	\addConstraint{ }{+ \int_0^1 \big( w_{RD,jt} + (\frac{\bar{q}_{jt}}{Q_t})^{-1} \nu (1-\kappa_e) V(j,t|\lambda \bar{q}_{jt}) \big) \ell_{RD,j}(t) dj}
...
\begin{maxi*}[0]
	{\{C(t)\}_{t\ge0}} {\int_0^{\infty} \frac{C(t)^{1-\theta}-1}{1-\theta} dt}{}{}
	\addConstraint{ C(t)}{ \ge 0} 
	\addConstraint{ A'(t)}{ = r_t A(t) + \bar{w}_t(1 - \bar{L}_{RD}) + \hat{w}_{RD,t} \bar{L}_{RD} - C(t)} 
	\addConstraint{\lim_{t \to \infty} e^{-\int_0^t r_s ds} A(t) }{\ge 0}
	\addConstraint{A(0)}{ = A_0} 
\end{maxi*}
