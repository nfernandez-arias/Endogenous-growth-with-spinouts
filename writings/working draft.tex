...
Because my analysis focuses on creative destruction by employee spinouts, it is crucial to distinguish spinouts in the same industry as their parents from those in a different industry. While the Venture Source data do not include NAICS codes, they do provide detailed industry information at a similar level of aggregation. The scheme is roughly consistent, as most Venture Source industry categories coincide with 4-digit NAICS codes. Therefore, I proceed by manually constructing a cross-walk.
...
Incumbents, entrants and spinouts hire R\&D labor in exchange for a chance of innovating on intermediate goods. An innovation on a good of quality $q$ grants the ability to produce a good of quality $\lambda q$, where $\lambda > 1$ is the exogenous step size on the quality ladder. These innovations drive aggregate productivity growth in the economy.

\subsubsection{Innovation by incumbents}

The current producer of the highest quality $q_j$ of good $j$ can hire $\big(\frac{q}{Q}\big)z_I$ units of R\&D labor in exchange for a Poisson arrival rate of innovations
\begin{align}
\tau_I &= \chi_I z_I^{1-\psi_I}  \label{incumbent_innovation_rate}
\end{align}
where $\chi_I > 0$ governs the overall productivity of incumbent R\&D and $\psi \in (0,1)$ governs the rate of decreasing returns. The assumption that innovations on goods of higher relative quality $q/Q$ are more costly is standard. In lab-goods models of innovation, the inputs required scale with $q$. Here, the specification is with $q/Q$ to account for the fact that labor is a factor that is not growing with the size of the economy. The linear specification adds significant tractability, as it means the value of an incumbent firm is linear in $q$, effectively eliminating a state variable. In equilibrium, this leads to a growth rate independent of $q$. 
...
\begin{align}
\tau_E^e &= \chi_E z_E^e (z_E + z_S)^{-\hat{\psi}} de \label{simplified_entrant_innovation_rate} \\
\tau_E &= \int_0^1 \tau_E^e \nonumber
\end{align}
...
\begin{align}
\tau_S^s &= \chi_S z_S^s (z_E+ z_S)^{-\hat{\psi}} ds \label{spinout_entry_rate_eq} \\
\tau_S &= \int_0^m \tau_S^s \nonumber
\end{align}

Spinouts have a capacity constraint $z_S^s \le \xi$, where $\xi$ can be normalized to $1$ without loss of generality as observables depend only on $\nu \xi$. This capacity constraint can be interpreted as a stark form of individually decreasing returns to scale. Some form of this assumption is necessary; otherwise, $m_j > 0$ is sufficient to obtain free entry and the dynamics of the model are trivialized. 

Note also that, given the individually CRS + capacity constraint specification, $\chi_S \le \chi_E$ implies that spinouts never enter and the model nests a standard quality ladders model with no spinouts (with labor as the R\&D input). This is no longer the case if this specification is replaced by a more flexible DRS function such as 
\begin{align}
\tau_S^s &= \chi_S \Big(z_S^s\Big)^{\alpha} (z_E+ z_S)^{-\hat{\psi}} ds \label{spinout_entry_rate_eq_DRS} 
\end{align}
...
The model gains significant tractability from the fact that production decisions are entirely separable from R\&D decisions. 
...
The state variable of a product line $j$ is $(q,m,t)$, where $m$ is the mass of spinouts capable of attempting entry and $t$ allows dependence on the aggregate distribution $d\mu(q,m,t)$ of lines in $(q,m)$-space at time $t$, including average quality $Q = \int_{q,m} q d\mu(q,m,t)$. 

Let $V(q,m,t)$ denotes the value of incumbency and let $W(q,m,t)$ denote the density of value per spinout idea. That is, the value of all spinout ideas in a line in state $(q,m,t)$ is $\int_0^m W(q,m',t) dm'$. 
...
&+ \max_{z \ge 0, x\in \{0,1\}} \Big\{ \underbrace{\chi_I z^{1-\psi_I} \Big[V(\lambda q,0,t) - V(q,m,t) \Big]}_{\textrm{Expected value of innovation}} \nonumber \\
...
Table \ref{identification} displays the various parameters to be identified alongside their sources of identification. The curvature parameters $\psi_I,\psi_{SE}$ and the creative destruction cost parameter $\kappa$ are important for the comparative statics in my model and are not tightly identified. Therefore, in the counterfactual analysis section, I will provide robustness exercises where I calibrate conditional on different values of these parameters.

\begin{table*}[h]\label{identification}
	\centering
	\begin{tabular}{lll}
		Table 4.1: Identification &   \tabularnewline
		\hline 
		Parameter &  Description & Source / match / target \tabularnewline
		& \tabularnewline
		\hline 
		Chosen outside model & \tabularnewline
		$\psi_I$ & Curvature of incumbent R\&D & Micro estimates \tabularnewline
		$\psi_E$ & Curvature of entrant, spinout R\&D & Micro estimates\tabularnewline
		&  \tabularnewline
		Calibrated directly &  \tabularnewline
		$\rho$ & Discount factor & Interest rate \tabularnewline
		$\beta$ & $\beta^{-1}$ EoS between intermediate goods & Profit / sales ratio \tabularnewline
		$\chi_S / \chi_E$ & Spinouts / entrants R\&D prod. & Spinout / entrant probability \tabularnewline
		& & of reaching profitablity \tabularnewline
		$\theta$ & Fraction of spinouts that & WSO reg. coef. divided \tabularnewline
		& are WSOs & by All Spinout reg. coef. \tabularnewline
		& \tabularnewline
		Indirect inference & \tabularnewline
		$\lambda$ & Step size of innovations & Growth rate \tabularnewline
		$\chi_I$ & Incumbent R\&D prod. & Fraction of patents by \tabularnewline
		& & incumbent firms  \tabularnewline
		$\kappa$ & Cost of creative destruction & Entry rate \tabularnewline
		$\chi_E$ & Entrant R\&D prod. & Entry rate \tabularnewline
		$\nu$ & Spinout idea generation rate & Spinout entry rate \tabularnewline
		\hline
	\end{tabular}
\end{table*}

Table \ref{calibration_targets} displays the calibrations targets in the data and in the model. The model matches the targetted moments well. This is not strong validation of the model since there are as many parameters as targeted moments, but it is a start. 


\begin{table*}[h]\label{calibration_targets}
	\centering
	\begin{tabular}{lll}
		...
		& \tabularnewline
		\hline 
		Growth rate $g$ & 0.02 & 0.01986 \tabularnewline
		Risk-adjusted interest rate $r$ & 0.05 & 0.05 \tabularnewline
		Profit / sales ratio & .109 & .109 \tabularnewline
		S/E profit. probability & 1.31 & 1.31 \tabularnewline
		\tabularnewline
		Internal patent share & 0.7 & 0.73
		\tabularnewline
		Entry rate & 0.10 & 0.104
		\tabularnewline
		Spinout entry rate & 0.025 & 0.0254
	\end{tabular}
\end{table*}
...
\begin{table*}[h]\label{calibration_parameters}
	...
	$\lambda$ & 1.051\tabularnewline
	$\chi_I$ & 4.492\tabularnewline
	$\chi_E$ & 0.727\tabularnewline
	$\chi_S$ & 0.953\tabularnewline
	$\kappa$ & 0.6 \tabularnewline
	$\nu$ & 0.010 \tabularnewline
	$\theta$ & 0.46
	\end{tabular}
\end{table*}
...
\begin{figure}[p]
	\centering
	\input{../empirics/tables/presentationRegressions_noFE.tex}
...
\begin{figure}[p]
	\centering
	\input{../empirics/tables/regs_accounting.tex}
...
\begin{figure}[p]
	\centering
	\input{../empirics/tables/spinoutCounts_regressions_accounting.tex}
	\caption{The dependent variable is cumulated over $t+1,t+2,t+3,t+4$ and controls are measured at $t$. R\&D is measured in billions of effective real units of R\&D, as in the scatter plots. All other variables are measured in 2012 US \$. The same controls are used as in Figure \ref{regs_allSpinouts_fe}.}
