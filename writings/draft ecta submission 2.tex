...
\begin{table}[]
	...
	\input{../empirics/figures/tables/RDandSpinoutFormation_headlineRegs2_small.tex}
...
\end{align}
The household takes as given profits it receives from the ownership of all firms in the economy. The household also has access to an instantaneous risk-free bond that exists in zero net supply. 

The above specification implies that the aggregate supply of R\&D labor is inelastic. This makes it possible to solve the model analytically. It also implies that productivity growth is determined only the allocation of R\&D across different uses, rather than by its amount. The model can be viewed in two ways. First, it can be considered as an approximation that is accurate in the medium term, i.e., when the total amount of R\&D labor is constrained by the slow process of human capital accumulation. Alternatively, it can be interpreted as a model that serves to highlight the effects of R\&D incentives on the allocation of R\&D labor to different uses rather than the total amount of R\&D labor supplied.\footnote{It would be straightforward to extend the model to a case with an endogenous aggregate supply of R\&D labor, at the cost of no longer having closed form expressions for the resulting equilibrium allocation. \textbf{Maybe get rid of this?} Assume that the household has an underlying stock of labor $\bar{L}$, normalized to $\bar{L} = 1$, of which it deploys $\tilde{L}_{RD}$ to the production of R\&D human capital with concave production function $f()$. The remainder can be used for production. The R\&D labor resource constraint (\ref{labor_resource_constraint2}) becomes $L_{RD} \le f(\tilde{L}_{RD})$ and production labor resource constraint (\ref{labor_resource_constraint}) becomes $L_P \le 1 - \tilde{L}_{RD}$. If $f(L) = aL, a > 0$, the supply of R\&D labor is infinitely elastic and R\&D wage is constrained to be a multiple of the production wage. If $f$ is strictly concave, one has a model with elastic, but not infinitely elastic, R\&D labor supply. In both cases, corporate profits, and hence the incentives for innovation, themselves depend on the amount of innovation in the economy. Computing equilibrium therefore requires finding a fixed point of this feedback loop and, as a result, there is no closed form solution.}
...
Each quality $q_{jti}$ of each good $j$ can be produced by a firm which has a monopoly on production of that quality of good $j$. The producer which is able to produce the frontier quality of good $j$ is denoted \emph{incumbent} $j$. The production function for an intermediate good $j$ of any quality is given by
\begin{align}
	k_{jti} = H(\ell_{jti};Q) &= Q \ell_{jti}, \label{intermediate_goods_production}
\end{align}
where $\ell_{jti} \ge 0$ is the production labor input and $Q_t = \int_0^1 \bar{q}_{jt} dj$ is the average frontier quality level in the economy. Note that this means that incumbent $j$ is able to produce a higher quality of good $j$ at the same cost as all other producers. There is no storage of intermediate goods. 

The scaling with average quality $Q_t$ implies that improvements in the quality of good $j$ increases the productivity of all other goods $j' \ne j$ through a knowledge spillover externality. Balanced growth requires the scaling to be linear in order to offset the fact that labor's marginal product (and hence the wage) increases linearly with average quality $Q_t$.

\paragraph{Intermediate goods market structure} The following setup, which allows for abstracting from limit pricing, is drawn from \cite{akcigit_growth_2018}. Within each good $j$, intermediate goods producers play a two-stage Bertrand competition game at each time $t \ge 0$. In the first stage, participants bear a cost of $\varepsilon > 0$ units of the final good in exchange for a right to compete in the second stage. Then, in the second stage, they engage in Bertrand competition against each other and against producers of goods $j' \ne j$. Optimal pricing under Bertrand competition in the second stage implies that all producers not on the frontier will have zero profits. By backward induction, such producers do not pay the $\varepsilon$ entry cost. The incumbent therefore has a second-stage monopoly over good $j$ which implies the optimal price is the monopolistic competition markup $(1-\beta)^{-1}$ over marginal costs. I study the limit of this model as $\varepsilon \to 0$.

\paragraph{Own-product innovation}\label{subsubsec:OI}

Incumbent $j$ can hire R\&D labor to improve the quality of the good she can produce. I refer to this as \textit{own-product innovation} or OI, following \cite{garcia-macia_how_2019} and \cite{klenow_innovative_2020}.\footnote{One could also use the term \textit{internal innovation}, following \cite{akcigit_growth_2018}.} By performing a flow of $z_{jt}$ units of R\&D, she innovates on good $j$ with Poisson arrival rate 
\begin{align}
	\tau_{jt} &= \chi z_{jt},
\end{align}
where $\chi > 0$ is an exogenous parameter representing the incumbent's R\&D productivity. A successful own-product innovation grants incumbent $j$ with quality $q_j$ a monopoly on production of good $j$ of quality $\lambda q_j$; that is, the frontier quality improves by an exogenous \emph{step size} $\lambda > 1$.  Note that the above specification exhibits constant returns to scale in R\&D spending. This is necessary to have a closed form solution but does not drive any of the results, either qualitatively or quantitatively (see Online Appendix \ref{appendix:policyanalysis:ncacost}).

When directed at a product of relative quality $\tilde{q}_{jt} \equiv \frac{\bar{q}_{jt}}{Q_t}$, the flow cost of $z_{jt}$ units of R\&D is $\tilde{q}_{jt}z_{jt}$ units of R\&D labor. This scaling assumption captures the idea that higher quality goods require more human capital to improve in quality. An assumption of this kind is also helpful for tractability, as it means that equilibrium is consistent with a constant own-product innovation rate across all intermediate goods. This allows for a analytic solution to the model and, in particular, obviates the need to ensure that the distribution of relative incumbent qualities $\tilde{q}_{jt}$ has a stationary distribution along a BGP (in the present model, it does not).\footnote{If equal innovation rates on different qualities is not consistent with equilibrium, then the overall growth rate depends on higher moments of the relative quality distribution. In order to be consistent with BGP, then, those moments must be constant (except perhaps on some knife-edge). In order for those moments to be constant, different individual dynamics for incumbent innovation must be assumed: specifically, a lower exit barrier for low quality incumbents plus offsetting entry of new incumbents at a relative quality which is a time-invariant function of the moments of the relative quality distribution. For more discussion of this type of question, see \cite{gabaix_power_2009} or \cite{acemoglu_innovation_2015}.} Note that it implies that there are knowledge spillovers in R\&D: an increase in the quality of good $j$ improves the R\&D technology of all other goods $j' \ne j$. 

I make the standard assumption that only incumbents can perform own-product R\&D. That is, when an incumbent is overtaken by an entrant or spinout innovation (described in the next section), she loses access to the OI R\&D technology and therefore cannot use it to ``catch up'' to the frontier. One interpretation is that learning by doing means the current producer of a product has unique insights into how to improve on it. This assumption also significantly increases tractability by reducing the dimensionality of the incumbent's state space.

\subsubsection{Spinouts and noncompete agreeements}\label{subsubsec:generation_of_spinouts}

R\&D spending by incumbents is the source of within-industry employee spinout formation. Specifically, I assume that a flow of R\&D spending on own-product innovation induces a proportional Poisson rate of being overtaken by an employee spinout if no noncompete is used. If the incumbent imposes a noncompete, the rate of within-industry spinout generation is zero. The decision of whether a noncompete is used is made instant-by-instant and prevents spinouts during the time it is used. Formally, I assume that if incumbent $j$ conducts $z_{jt}$ units of R\&D effort, she faces a Poisson intensity of spawning a spinout given by 
\begin{align}
	\tau^S_{jt} &= (1-\mathbbm{1}^{NCA}_{jt}) \nu z_{jt}, \label{def:tau_S}
\end{align} 
where $\mathbbm{1}^{NCA}_{jt} = 1$ if and only if an NCA is used at instant $t$. The contract is renegotiated instant-by-instant in the sense that the employee takes as given the incumbent's NCA policy and wage offered when deciding whether to supply R\&D in each instant. Therefore, the incumbent cannot use an NCA without potentially having to offer a higher wage to compensate the employee for the fact that he will not be able to profit from WSO formation.

An employee spinout from incumbent $j$ of quality $\bar{q}_{jt}$ is able to produce good $j$ with quality $\lambda \bar{q}_{jt}$. Therefore, it immediately becomes the new incumbent and the previous incumbent's profits go to zero forever after. The exogenous parameter $\nu \ge 0$ encodes the rate at which R\&D increases the likelihood of replacement by a WSO. As such, it determines the strength of the key mechanism documented in the preceding empirical section. Recall that $z_{jt}$ applied to a good of relative quality $\frac{q}{Q_t}$ requires $\frac{q}{Q_t}z_{jt}$ units of R\&D labor. Therefore, this specification implies that the rate of spinout generation of a unit of R\&D labor is inversely proportional to the relative quality $\frac{q}{Q_t}$ of the good upon which it seeks to innovate, consistent with the assumption above on the own-product innovation technology.

In order to enter, employee spinouts must pay an entry cost of $\kappa_{e} V(j,t|\lambda \bar{q}_{jt})$ units of the final good in order to begin producing, where $\kappa_e \in [0,1)$ is exogenous and $V(j,t|\lambda \bar{q}_{jt})$ denotes the equilibrium private value of incumbent $j$ at time $t$ with quality $\lambda \bar{q}_{jt}$. This cost represents non-R\&D expenditures required by creative destruction innovation but not by own-product innovation, such as marketing costs or the costs of setting up a new firm.\footnote{On the other hand, these expenditures could also reflect unmodeled frictions (e.g., asymmetry of information) in the market for ideas which prevent entrants and spinout entrepreneurs from appropriating the full value of their ideas. In this interpretation, entry by spinouts is more socially valuable. Hence, the choice of interpretation does have implications for welfare analysis. The data I will use to calibrate the model will not be able to distinguish between these two interpretations. Therefore, I proceed assuming that they are real costs and consider the transfers interpretation in robustness checks.} The scaling of the cost with $V(j,t|\lambda q_{jt})$ is natural as it simply means that the cost of entry is proportional to the value of the incumbency position obtained through entry. Further, this scaling assumption is important for the closed form solution to the model. 

Using an NCA requires the incumbent to pay a flow cost. Specifically, when incumbent $j$ imposes an NCA on $z_j$ units of R\&D, she must pay a flow cost $\kappa_{c} \nu V(j,t|\bar{q}_j) z_j$ units of the final good. Given (\ref{def:tau_S}), incumbent $j$ overall pays
\begin{align}
	\textrm{NCA cost}_{jt} &= \tau^S_{jt} \kappa_c V(j,t|q). \label{def:nca_cost}
\end{align}
The NCA cost reflects the direct cost using an NCA, including legal barriers to their use. If there are no legal restrictions on what kinds of NCAs are valid, the incumbent needs to prove in court that the startup competes with it. Moreover, many jurisdictions do, in fact, impose such restrictions. In these cases where those restrictions do not apply, resources must be invested to prove that in a court. Finally, an infinite cost $\kappa_c = \infty$ represents a complete ban on the use of noncompetes.  
...
Finally, I want to make explicit an assumption implicit in (\ref{def:tau_S}). I have assumed spinout entry does not directly reduce the rate at which incumbent R\&D results in successful OI. Instead, spinouts enter when an additional, independent Poisson process with arrival rate $\nu z_j$ has an arrival. In other words, spinouts in this model do not steal ideas that otherwise would have been implemented by the parent firm. Rather, R\&D labor generates \textit{additional} innovations which the employee can use to offer a higher quality product than the incumbent firm. This is consistent with the finding in \cite{klepper_disagreements_2007} that spinouts often occur when the employee finds the idea more valuable than his employer rather than through idea stealing. This assumption has important consequences for the private and social usefulness of NCAs. To the extent that spinouts take ideas that otherwise would be implemented inside the firm, they are less valuable both privately and socially. In the context of this model and calibration, the socially beneficial effects of NCA enforcement would be larger.\footnote{The validity of this assumption could be tested empirically with exogenous variation in the enforceability of NCAs or in the use of NCAs. One could study, for example, whether R\&D generates fewer patents when R\&D managers and employees are not bound by NCAs. However, as noted in the introduction, interpreting the results of regressions using even exogenous variation in enforcement requires care to control for the effects of mobility across enforcement regions; and exogenous variation in the use NCAs at the firm- or employee-level is simply not available; in reality, it may correlated with firm or employee characteristics in a way that could lead to a spurious correlation.}

\subsubsection{Entrants} \label{subsubsec:entrants}

In addition to own-product innovation and spinout formation, growth also results from creative destruction by entrant firms. For each frontier quality good $j$ there is a unit mass of entrants indexed by $e \in [0,1]$.\footnote{As is standard in this type of model, I assume for simplicity that entrants innovate only on frontier quality goods.} By performing a flow of $\hat{z}_{jet}$ units of R\&D, an entrant receives a Poisson intensity of $\hat{z}_{jet} \hat{\chi} \bar{\hat{z}}_{jt}^{-\psi}$ of innovating on good $j$, where $\bar{\hat{z}}_{jt} = \int_0^1 \hat{z}_{jet} de$ total entrant R\&D dedicated improving good $j$. From now I drop the bar notation when it is clear to which I refer (based on the number of subscripts). Because entrants are infinitesimal, their R\&D effort has constant returns to scale at the individual level. Note that this means entrant optimization will only pins down $\hat{z}_{jt}$ in equilibrium; as this does not affect aggregates, I will assume that $\hat{z}_{jet} = \hat{z}_{jt}$. Aggregating over $e \in [0,1]$, the arrival rate of entrant innovations on good $j$ is 
\begin{align}\label{model:entrantsInnovationTechnology}
	\hat{\tau}_{jt} &= \hat{\chi} \hat{z}_{jt}^{1-\psi}.
\end{align}
As with the other forms of innovation, the cost in terms of R\&D labor is proportional to the relative quality of the good $\frac{\bar{q}_{jt}}{Q_t}$ and a successful innovation yields a monopoly on the production of good $j$ with quality $\lambda \bar{q}_{jt}$. The parameter $\psi > 0$ introduces decreasing returns at the level of good $j$. It represents a \textit{congestion} externality in the entrant innovation technology. This is similar to the congestion externality present in search and matching models. Intuitively, due to a lack coordination, entrants attempt similar approaches to solve the same problem, reducing their overall efficiency. 

Finally, as with spinouts, entrants pay an entry cost of $\kappa_{e} V(j,t|\lambda \bar{q}_{jt})$ units of the final good once they have successfully innovated on a good of quality $\bar{q}_{jt}$. The interpretation is the same as with spinouts.

\subsection{Competitive financial intermediary}\label{model:financial_intermediary}

The following construction implies that the representative household does not take into account the loss of value to the incumbent when valuing spinouts it forms. I assume that the representative household owns a competitive financial intermediary which in turn owns all firms in the economy and remits their profits back to the representative household. Individual firms in the economy maximize profits subject to the household's risk-free discount rate, i.e., there is no collusion due to common ownership. When the representative household receives a shock in good $j$ that allows it to form a spinout, it sells the spinout to the financial intermediary at full private value (i.e. discounting the spinouts profits at the same risk-free discount rate). The financial intermediary takes the entry of the spinout as given, and therefore is willing to pay this value even though the entry of the spinout reduces the value of an existing incumbent. 
...
Symmetric BGPs are a natural type of equilibrium given the symmetric setup of the model. It is the typical case studied, e.g. in \cite{grossman_quality_1991} and \cite{acemoglu_innovation_2015}.
...
\end{maxi*}
There are also non-negativity constraints on consumption and labor supply. Thus, the household consumes out of profits remitted by the intermediary $\Pi_t$, returns from holdings of the risk free bond $r_t A(t)$, wages earned from production labor supply $\bar{w}_t L(t)$, wages earned from R\&D labor supply $\int_0^1 \big(w_{RD,jt} \ell_{RD,j}(t) + \hat{w}_{RD.t} \hat{\ell}_{RD,j}(t) \big) dj$, and earnings from sales of spinouts to the financial intermediary $\int_0^1 (1-\mathbbm{1}^{NCA}_{jt}) (\frac{\bar{q}_{jt}}{Q_t})^{-1} \nu (1-\kappa_e) V(j,t|\lambda \bar{q}_{jt}) \big)\ell_{RD,j}(t) dj$. The expression for the value of earnings from sales of spinouts follows because a spinout occurs in good $j$ with Poisson intensity $(1-\mathbbm{1}^{NCA}_{jt}) (\frac{\bar{q}_{jt}}{Q_t})^{-1} \nu \ell_{RD,j}(t)$ and is sold to the financial intermediary at the price of $(1-\kappa_e) V(j,t|\lambda \bar{q}_{jt})$.\footnote{To be more precise, the household forms, and sells, a spinout in each good $j$ upon the arrival of an independent random count process $N_{jt}$ with time-varying hazard rate $(1-\mathbbm{1}^{NCA}_{jt} ) \nu (\frac{\bar{q}_{jt}}{Q_t})^{-1} \ell_{RD,jt}$. The expected payoff of spinouts from good $j$ is therefore $(1-\mathbbm{1}^{NCA}_{jt} ) \nu (\frac{\bar{q}_{jt}}{Q_t})^{-1} \ell_{RD,jt} (1-\kappa_e) V(j,t|\lambda \bar{q}_{jt}) dj dt$. The budget constraint integrates over $j \in [0,1]$ and divides by $dt$.}



Optimal savings gives rise to a standard Euler equation which holds in equilibrium for all $t \ge 0$, given by
\begin{align}
...
There is also a transversality condition on financial wealth $A(t)$; however, in this model it holds trivially since $A(t) = 0$ in equilibrium. The usual transversality condition, which would be imposed here if the household traded claims on the financial intermediary, appears later as a necessary and sufficient condition for finiteness of household utility.

\paragraph{Entrant optimization}

Entrant $e$ in good $j$ chooses $\hat{z}_{jet}$ to maximize profits, taking as given prices (i.e. the R\&D wage, the interest rate), good $j$ entrant R\&D $\hat{z}_{jt}$, and the value of being an incumbent in good $j$ at time $t$ with quality $\lambda \bar{q}_{jt}$, denoted $V(j,t|\lambda \bar{q}_{jt})$. The resulting optimization problem is
\begin{align}
...
\end{align}
where, as before, $\hat{z}_{jt} = \int_0^1 \hat{z}_{jet} de$. 
...
\begin{proof}	
	The first-order condition corresponding to (\ref{eq:entrant_optimization_problem}) is
	\begin{align}
	...
	\end{align}
	If $\hat{z}_{jt} = 0$ the left hand-side is $+\infty$. The right-hand side, however, is necessarily finite. Therefore entrant $e$ optimally chooses $\hat{z}_{jet} = +\infty$ and, integrating over $e$, one obtains $\hat{z}_{jt} = +\infty > 0$, a contradiction.  
\end{proof}

\paragraph{Incumbent optimization}

Incumbent $j$ takes as given flow profits $\pi(j,t|q) = \tilde{\pi} q$ as derived in the static equilibrium, the interest rate $r_t$, the rate of creative destruction by entrants $\hat{\tau}(j,t|q)$, and the R\&D wage she must pay conditional on her choice of NCA policy $w_{RD}(j,t|q , \mathbbm{1}^{NCA})$. Note that the discounting is at the risk-free rate $r_t$ because the financial intermediary diversifies across incumbents and there is no aggregate uncertainty. 

Because the R\&D labor market is competitive, the incumbent takes as given that she can hire as much R\&D labor as necessary provided that she offers total compensation equal to that offered by entrants. If she does not use an NCA, this compensation includes the expected value of spinouts formed by the household. 
...
\begin{proof}
	The expression on the left-hand side is equal to the expected flow compensation to a unit of R\&D labor employed in own-product innovation at good $j$ of quality $q$ with noncompete policy $\mathbbm{1}^{NCA}$, including the expected value of future spinouts. The right-hand side is the equilibrium price of R\&D labor, as $\hat{z} > 0$ by Lemma \ref{lemma:positive_entry}. 
\end{proof}

The above discussion implies that the value of an incumbent $V(j,t|q)$ satisfies the following Hamilton-Jacobi-Bellman equation:
\begin{align}
...
The proposition below shows that in a symmetric BGP the value of incumbency in good $j$ depends only on the quality of good $j$, and that it does so linearly.
...
Proposition \ref{proposition:hjb_scaling} has the following corollary.
...
\begin{proof}
	Use Proposition \ref{proposition:hjb_scaling} to substitute $V(j,t|\lambda q) = \tilde{V} \lambda q$ into the entrant first-order condition and divide by $\bar{q}_{jt}$ to obtain
	\begin{align}
		\hat{\chi} \hat{z}^{-\psi} \tilde{V} \lambda &= \frac{\hat{w}_{RD}(t)}{Q_t},
	\end{align}
	implying that $\frac{\hat{w}_{RD}(t)}{Q_t}$ must be constant, i.e. $\hat{w}_{RD}(t) = \tilde{\hat{w}}_{RD} Q_t$ for some $\tilde{\hat{w}}_{RD}$. Using Lemma \ref{lemma:RD_worker_indifference1} and Proposition \ref{proposition:hjb_scaling} again, this time to obtain $V(j,t|q) = \tilde{V}q$, yields $w_{RD}(j,t|q, \mathbbm{1}^{NCA}) = \tilde{w}_{RD}(\mathbbm{1}^{NCA}) Q_t$. 
\end{proof}
...
Proposition \ref{proposition:optimalNCApolicy} implies that, in a symmetric BGP, all incumbents choose the same NCA policy, except on the knife-edge $\kappa_c = \bar{\kappa}_c$ where incumbents are indifferent between $\mathbbm{1}^{NCA} = 0$ and $\mathbbm{1}^{NCA} = 1$. The proof follows from the equation below, which is obtained by substituting into the incumbent HJB (\ref{eq:hjb_incumbent_0}) the expression for the incumbent R\&D wage in Lemma \ref{lemma:RD_worker_indifference1}.\footnote{This results from the assumptions that the cost of using an NCA is proportional to incumbent quality and that incumbents differ only in their quality $q$. The model can be extended to accomodate exogenous heterogeneity in $\kappa_c$ and $\kappa_e$, which would imply a fraction of incumbents would use NCAs. However, there would be no closed form solution and I do not have a clear way to discipline the properties of this distribution. Further, extending the model in this way would likely replacing the own-product innovation technology with one that has decreasing returns to scale. Otherwise, heterogeneity in $\kappa_e$ and $\kappa_c$ would generally imply that only a subset of incumbents conduct R\&D, complicating the solution to the model.}
...
The final two terms determine the incumbent's optimal NCA policy. The term $-(1-\mathbbm{1}^{NCA})(1 - (1-\kappa_e) \lambda) \nu \tilde{V}$ is the expected net cost to the incumbent per unit of R\&D associated with not using an NCA. It consists of two components: $-(1-\mathbbm{1}^{NCA})\nu \tilde{V} $ is the expected capital loss from creative destruction by employee spinouts per unit of R\&D and $(1-\mathbbm{1}^{NCA})(1-\kappa_e)\lambda \nu \tilde{V}$ is the equilibrium R\&D wage discount when an NCA is not imposed. The latter is positive because, in equilibrium, the R\&D worker accepts a lower wage in return for an expected future payoff from spinout formation, by Lemma \ref{lemma:RD_worker_indifference1}. The term $-\mathbbm{1}^{NCA} \kappa_c \nu \tilde{V}$ reflects is the direct cost of using an NCA. Incumbents optimally the value of $\mathbbm{1}^{NCA}$ which maximizes the term multiplying $z$, yielding Proposition \ref{proposition:optimalNCApolicy}. 
...
Using the above results, one can characterize the set of possible symmetric BGPs. First, suppose that $\kappa_c \ne \bar{\kappa}_c$. Then Proposition \ref{proposition:optimalNCApolicy} yields the unique noncompete policy consistent with equilibrium. Next, using Proposition \ref{proposition:hjb_scaling} and its corollary, the first-order condition (\ref{eq:entrant_optimiziation_foc}) for the entrant's optimization problem becomes
\begin{align}
	\hat{\chi} \hat{z}^{-\psi} (1-\kappa_e) \lambda \tilde{V} = \hat{w}_{RD} \label{eq:free_entry_condition}
\end{align}
Using Lemma \ref{lemma:hjb_incumbent_foc} to substitute for $\tilde{V}$ yields an expression for entrant R\&D, 
\begin{align}
...
\end{align}
Suppose that (\ref{eq:zI_asFuncZe}) is consistent with $z > 0$. Then growth is determined by the growth accounting equation\footnote{A derivation can be found in Online Appendix \ref{appendix:model:growth_accounting_equation}.}
\begin{align}
...
\end{align}
Substituting the incumbent's first-order condition and (\ref{eq:interest_rate}) into the HJB (\ref{eq:hjb_incumbent_workerIndiff}) and solving for $\tilde{V}$ yields
\begin{align}
	\tilde{V} &= \frac{\tilde{\pi}}{r + \hat{\tau}}. \label{eq:hjb_incumbent_gordon_formula}
\end{align}
The entrant optimality condition (\ref{eq:free_entry_condition}) determines the equilibrium value of $\hat{w}_{RD}$. If (\ref{eq:zI_asFuncZe}) is inconsistent with $z > 0$, then the derivation is the same except that (\ref{eq:effort_entrant}) and (\ref{eq:zI_asFuncZe}) are replaced with $\hat{z} = \bar{L}_{RD}$ and $z = 0$.

Finally, I state two assumptions. The first assumption characterizes when $z > 0$ and when $z = 0$ in a symmetric BGP.
...
The second assumption characterizes when household utility is finite in a symmetric BGP.

\begin{assumption}\label{model:assumption:boundedUtility1}
	$\rho > (1-\theta) g$.
\end{assumption} 

In Assumption \ref{model:assumption:boundedUtility1}, $g$ stands for the closed form expression for equilibrium growth $g$. In particular, if Assumption \ref{ineq:zhat_market_clearing} holds then $z > 0$ and
\begin{align}
	g &= (\lambda - 1) \Bigg(  \Big( \frac{\hat{\chi} (1-\kappa_e) \lambda}{\chi(\lambda-1) - \nu \min \{1 - (1-\kappa_e) \lambda, \kappa_c \}} \Big)^{(1-\psi)/\psi)} \\
	&+ \big(\chi + (1- \mathbbm{1}_{\kappa_c < \bar{\kappa}_c})\nu \big) \Big( \bar{L}_{RD} - \big( \frac{\hat{\chi} (1-\kappa_e) \lambda}{\chi(\lambda-1) - \nu \min \{1 - (1-\kappa_e) \lambda, \kappa_c \}} \big)^{1/\psi} \Big) \Bigg).
\end{align}
Otherwise,
\begin{align}
...
\begin{lemma}
	The household's utility is finite on a symmetric BGP if and only if Assumption \ref{model:assumption:boundedUtility1} holds.
\end{lemma}
...
Note that $\theta \ge 1$ implies Assumption \ref{model:assumption:boundedUtility1}. This is the case which I consider in the quantification of the model. 
...
The model has 11 parameters, $\{\rho, \theta, \beta, \psi, \lambda, \chi, \hat{\chi}, \kappa_e, \kappa_c, \nu, \bar{L}_{RD}\}$. The two elasticity parameters $\theta, \psi$ are set to standard choices from the literature.\footnote{I consider robustness of the main result to the value of these and all other parameters in Online Appendix \ref{appendix:policyanalysis:ncacost}.} The intertemporal elasticity of substitution parameter $\theta$ is set to 2, corresponding to a standard value used in the the literature. The entrant R\&D elasticity parameter $\psi$ is set to a value of 0.5 (the same as in \cite{acemoglu_innovation_2015}), corresponding to a quadratic cost.

The remaining nine parameters are calibrated to match moments. The parameter $\bar{L}_{RD}$ is calibrated to data on the share of total employment in R\&D. The parameter $\beta$ is set to match the profit to GDP ratio. The final seven parameters $\{\rho, \lambda, \chi, \hat{\chi}, \kappa_e, \kappa_c, \nu\}$ pertain to preferences ($\rho$) and to the innovation technology and NCA usage (all others), and are chosen to match six moments from the data, described in the next subsection. The parameter $\kappa_c$ is partially identified based on the observation that $\tau^S > 0$. The remaining six parameters are exactly identified and the model reproduces the target moments exactly. I discuss the sources of identification in Section \ref{subsec:identification}. 
...
Matching the productivity growth rate, R\&D to GDP ratio and growth share of OI captures the efficiency of R\&D in generating aggregate productivity growth through OI and CD. The real return, profit to GDP ratio and employment share of entering firms determines the discount factor and the reward to innovation. Further, the employment share of young firms helps identify the size of each innovation step size: at a constant contribution to aggregate growth, a lower share of employment at entering firms implies each innovation by an entering firm is larger. Finally, matching the employment share of entering WSOs captures the rate at which incumbent R\&D leads to spinout formation. 

Below, I discuss the calibration targets in more detail. I describe how to compute the equivalent model moments in Appendix \ref{appendix:calibration}.
...
The data the profit to GDP ratio comes from the BEA. I use the average ratio during the sample period of 1984-2006 of 8.5\%. 

\paragraph{Employment share of young firms}

As discussed in \cite{klenow_innovative_2020}, adjustment costs mean that it may take several years for a new product to displace an old one. But in the model, entrants that replace incumbents reach their mature size immediately upon entry. Therefore, if the model matches the amount of employment in firms of age < 1, it might underestimate the true growth impact of employment reallocation to new firms. For this reason, I choose to target the employment share of firms age $\le 6$. 

Second, because all entry in the model is creative destruction, including employment in entrants developing new varieties would overstate the rate of creative destruction. Therefore, I compute the employment of young firms as a share of the employment of incumbents and young firms engaging in creative destruction. To do this, I use the estimates in \cite{garcia-macia_how_2019} and \cite{klenow_innovative_2020}. They find that about 18\% of employment is in firms age $\le 6$ during the sample period, and that between 30\% and 70\% of the growth from these firms is due to creative destruction, the rest due to new variety creation. Importantly, in their framework, a given amount of growth from creative destruction requires more employment than a given amount of growth from new variety creation. Using a hypothetical value of $\lambda = 1.2$, for example, creative destruction requires 6 times more employment than new variety creation to generate the same amount of growth. Lower values of $\lambda$ imply a larger adjustment. Using $\lambda = 1.2$ as a benchmark value, I calculate an employment share of young firms of 13.34\% during the sample period. The calibrated value of $\lambda$ will be 1.084, so a choice of $\lambda = 1.2$ in the calculation of this target is conservative. A lower choice here implies a higher share of employment in young firms and, as discussed in the robustness analysis of Online Appendix \ref{appendix:policyanalysis:ncacost}, this would strengthen my main result. The computation of the corresponding model moment is described in \ref{appendix:calibration:entryRate}.
...
Using $z > 0$\footnote{As in the baseline model, it is straightforward to state a modified version of Assumption \ref{ineq:zhat_market_clearing} that characterizes when $z = 0$ or $z > 0$ in a symmetric BGP. For simplicity, I only consider the case $z > 0$ here.} and the incumbent FOC as before, the equilibrium R\&D allocation is
\begin{align}
	...
	z &= \bar{L}_{RD} - \hat{z}.
\end{align}
Inspection of the expression for $\hat{z}$ shows that, when $\mathbbm{1}^{NCA} = 0$, $\hat{z}$ increases in $T_{RD}$. Intuitively, an increase in the R\&D subsidy from $T_{RD}^0$ to $T_{RD}^1$ reduces the wage expenses paid for R\&D by the same factor $1-\frac{1-T_{RD}^1}{1-T_{RD}^0}$ for both incumbents and entrants. However, the incumbent's perceived cost of R\&D also includes the expected capital loss from creative destruction by an employee spinout, and this cost is not subsidized. Therefore, the incumbent's overall cost of R\&D is reduced by a smaller factor $\tilde{\tau}_{RD} < 1-\frac{1-T_{RD}^1}{1-T_{RD}^0}$. In general equilibrium, an increase in $T_{RD}$ results in R\&D labor being reallocated to entrants. Furthermore, as mentioned above, for large enough subsidies, $\kappa_c < \tilde{\bar{\kappa}}_c$ as the latter is increasing in $T_{RD}$, so the incumbent uses NCAs. Intuitively, with high R\&D subsidies the incumbent prefers to pay R\&D employees with subsidized wages rather than through future employee spinouts, the perceived cost of which is not subsidized.
...
Because the supply of R\&D labor is inelastic in this model, it is not surprising that untargeted R\&D subsidies are do not lead to higher growth rates. However, in a model without within-industry spinouts, untargeted R\&D would simply have no effect on the equilibrium R\&D allocation and hence growth. By contrast, this exercise shows that in a model with WSOs and NCAs, the general equilibrium adjustment of R\&D wages can lead to a counterproductive misallocation of R\&D in response to R\&D subsidies. This mechanism will play some role as long as the aggregate supply of R\&D labor is not perfectly elastic. It implies that untargeted R\&D subsidies have a weaker positive effect (less ``bang for buck'') than in a standard model without within-industry spinouts and NCAs. Finally, note that this mechanism also relies on the fact that firms with less to lose from spinout formation (entrants) engage in forms of R\&D that are inefficient at the margin.
...
\begin{align}
	...
	z &= \bar{L}_{RD} - \hat{z}.
\end{align}
When $\mathbbm{1}^{NCA} = 1$, it follows immediately that $\hat{z}$ is decreasing in $T_{RD,I}$. When $\mathbbm{1}^{NCA} = 0$, the term $T_{RD,I}$ appears in both the numerator and denominator with the same negative sign. However, it is still the case that $\hat{z}$ is decreasing in $T_{RD,I}$. To see this formally, raise (\ref{eq:effort_entrant_RDsubsidyTargeted_maintext}) to the power $\psi$ and differentiate with respect to $T_{RD,I}$. After some simplification, this yields
\begin{align}
	\frac{d\hat{z}^{\psi}}{dT_{RD,I}} &= \mathcal{K} \Big( -\chi(\lambda -1) + \nu \Big) 
\end{align}
where the strictly positive factor $\mathcal{K}$ depends on model parameters and $T_{RD,I}$. As exponentiation by $\psi$ is a monotonically increasing function, it follows that $\frac{d\hat{z}}{dT_{RD,I}}$ is strictly negative if and only if 
\begin{align}
	\chi(\lambda -1) > \nu. \label{cs:targeted_rd_subsidy_improvement_condition}
\end{align}
The condition (\ref{cs:targeted_rd_subsidy_improvement_condition}) holds in the calibration. In fact, it is a necessary condition for $\tau^S > 0$ in any symmetric BGP.\footnote{If it did not hold, incumbents would not be willing to hire any R\&D workers not bound by a noncompete for any positive wage.}
...
\begin{align}
	...
	z &= \bar{L}_{RD} - \hat{z}.
\end{align}
In this case, the R\&D allocation is more sensitive because there is no dampening secondary effect and there is no change in the NCA usage threshold $\hat{\bar{\kappa}}_c$. I focus on the subsidy to incumbent R\&D for two reasons. First, it may be impossible to implement at tax on R\&D as firms can simply claim that they are not conducting R\&D. The regulator would need to verify all of firms' expenses in search of hidden R\&D expenditures. Contrast that with the case of R\&D subsidies, which require the regulator only to evaluate the validity of firms' claimed R\&D expenses. Second, the equivalence of these two policies would no longer hold in a model with an elastic total supply of R\&D labor. In that case, a tax on R\&D, even if targeted, would reduce the total amount of R\&D. 
...
While the targeted R\&D subsidy analyzed in the last section can increase growth and welfare significantly, it also induces the use of NCAs, dampening its positive effect on growth and welfare. This suggests that the optimal policy involves positive targeted R\&D subsidies $T_{RD,I}$ and an increase in the cost $\kappa_c$ of NCA enforcement. In this section I consider the effect of such a joint policy.\footnote{I omit the derivation in this case for the sake of brevity, due to its similarity to the previous two cases.}
...
\begin{proof}
	...
	\begin{align*}
		(r + \hat{\tau}) \tilde{V} = \tilde{\pi} + \max_{\substack{\mathbbm{1}^{NCA} \in \{0,1\} \\ z \ge 0}} &\Bigg\{ z \Big( \chi (\lambda -1) \tilde{V}- w_{RD}(\mathbbm{1}^{NCA}) \\
		&- (1-\mathbbm{1}^{NCA}) \nu \tilde{V} - \mathbbm{1}^{NCA} \kappa_c \nu \tilde{V} \Big)\Bigg\}.
	\end{align*}
	Substituting in $w_{RD}(\mathbbm{1}^{NCA})$ using the indifference condition (\ref{eq:RD_worker_indifference}) derived in Lemma \ref{lemma:RD_worker_indifference} yields
	\begin{align*}
		(r + \hat{\tau}) \tilde{V} = \tilde{\pi} + \max_{\substack{\mathbbm{1}^{NCA} \in \{0,1\} \\ z \ge 0}} &\Big\{z \Big( \overbrace{\chi (\lambda - 1) \tilde{V}}^{\mathclap{\mathbb{E}[\textrm{Benefit from R\&D}]}}- \hat{w}_{RD} \\
		&-  \underbrace{(1-\mathbbm{1}^{NCA})(1 - (1-\kappa_{e})\lambda)\nu \tilde{V}}_{\mathclap{\text{Net cost from spinout formation}}} - \overbrace{\mathbbm{1}^{NCA} \kappa_{c} \nu \tilde{V}}^{\mathclap{\text{Direct cost of NCA}}}\Big) \Big\}.
	\end{align*}
	If $z > 0$, the incumbent maximizes her flow payoff by choosing $\mathbbm{1}^{NCA} \in \{0,1\}$ which maximizes the term multiplying $z$. Therefore, $\mathbbm{1}^{NCA} = 1$ is strictly preferred if $1 - (1-\kappa_e) \lambda > \kappa_c$ and $\mathbbm{1}^{NCA} = 0$ is strictly preferred if $1 - (1-\kappa_e) \lambda < \kappa_c$, yielding (\ref{eq_nca_policy}) with $\bar{\kappa}_c \coloneqq 1 - (1-\kappa_e)\lambda$. Finally, if $\kappa_c = \bar{\kappa}_c$, then it follows from the above expression that incumbents are indiferent between $\mathbbm{1}^{NCA} = 1$ and $\mathbbm{1}^{NCA} = 0$. 
\end{proof}
...
The solution to the static equilibrium implies $\pi_t / Y_t = \beta (1-\beta)$. Note that this function is not invertible, as $\beta = \beta_0$ and $\beta = 1 - \beta_0$ yield the same result. I choose the lower value of $\beta$ to avoid highly counterfactual implications for the average markups of intermediate goods.
...
\begin{figure}[]
	\centering
	\includegraphics[scale = 0.45]{../code/julia/figures/simpleModel/identificationSourcesFull_All.pdf}
