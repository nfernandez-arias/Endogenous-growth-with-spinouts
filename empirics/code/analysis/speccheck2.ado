...
capture program drop speccheck2

program speccheck2 // define the command's call
...
else if "`method'" == "reghdfe" {
	reghdfe `yvar' `xvar' `xvar1' `xvar2' `tuple`i'' `if', absorb(`absorb') cluster(`vce')
	}
else if "`method'" == "ppmlhdfe" {
	ppmlhdfe `yvar' `xvar' `xvar1' `xvar2' `tuple`i'' `if', absorb(`absorb') cluster(`vce')
	}
...
caption( ///
"`method' `varlist' `if', absorb(`absorb') cluster(`vce') always(`always')" ///
