...
foreach var of varlist lemployeecount lpostvalusd lrevenue goingoutofbusiness successfullyexiting {
...
foreach var of varlist lemployeecount_founder2 lpostvalusd_founder2 lrevenue_founder2 goingoutofbusiness successfullyexiting {
	...
	eststo: quietly reghdfe `var' entity_founder2_wso4, noabsorb cluster(statecode naics1_4 year)
	...
	esttab using "figures/tables/startupLifeCycle_founder2founders_regs_`var'_overall.tex", replace se star(* 0.1 ** 0.05 *** 0.01) keep(_cons *entity_founder2*) label nomtitles indicate(`r(indicate_fe)') stats(clustvar r2_a r2_a_within N, labels(Clustering "R-squared (adj.)" "R-squared (within, adj)" Observations)) interaction(" $\times$ ") style(tex) booktabs b(a2)
	esttab using "figures/tables/startupLifeCycle_founder2founders_regs_`var'_overall.csv", replace se star(* 0.1 ** 0.05 *** 0.01) keep(_cons *entity_founder2*) label nomtitles indicate(`r(indicate_fe)') stats(clustvar r2_a r2_a_within N, labels(Clustering "R-squared (adj.)" "R-squared (within, adj)" Observations)) interaction(" $\times$ ")
