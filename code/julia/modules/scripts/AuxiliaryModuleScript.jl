...
function zEFunc(modelPar::ModelParameters,V0::Float64,zI::Array{Float64},w::Array{Float64},zS::Array{Float64})
    ...
    if modelPar.spinoutsSamePool == true
        ...
        zE = max.( (wbarFunc(modelPar.β) ./ (modelPar.χE * (modelPar.λ .* V0 - modelPar.ζ))).^(-1/modelPar.ψI) .- zS .- zI,0)

    else

        zE = max.( (wbarFunc(modelPar.β) ./ (modelPar.χE * (modelPar.λ .* V0 - modelPar.ζ))).^(-1/modelPar.ψSE) .- zS,0)
