...
using EndogenousGrowthWithSpinouts_SimpleModel_IncumbentDRS

#-------------------------------#
# Set initial parameter settings, guess
#-------------------------------#

modelPar = initializeModelParameters()
algoPar = initializeAlgorithmParameters()

#-------------------------------#incu
# Enter calibration targets
#-------------------------------#

interestRate = CalibrationTarget(0.0857,1)
growthRate = CalibrationTarget(0.01487,1)
growthShareOI = CalibrationTarget(0.65,1)
youngFirmEmploymentShare = CalibrationTarget(0.1334,1)
spinoutEmploymentShare = CalibrationTarget(0.1,1)
rdShare = CalibrationTarget(0.0135,1)

calibPar = CalibrationParameters(interestRate,growthRate,growthShareOI,youngFirmEmploymentShare,spinoutEmploymentShare,rdShare)

#-------------------------------#
# Run calibration
#-------------------------------#

outerGuess = Guess(0,0)

@time calibrationResults,finalMoments,sol,finalScore = calibrateModel(modelPar,algoPar,calibPar,outerGuess)
